...
const equipeFiltrada = urlParams.get("equipe");

// SE O LINK VIER DO WHATSAPP, FORÇA A SESSÃO IMEDIATAMENTE NO NAVEGADOR
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Atualiza o nome do usuário no topo de forma segura
  const elementoUsuario = document.querySelector(".usuario");
  if (elementoUsuario) {
    elementoUsuario.innerHTML =
      nivelAcessoAtual === "montador"
        ? `👁️ Modo Visão (Montador) ▼`
        : (localStorage.getItem("nomeUsuario") || "Lima") + " ▼";
  }

  // Modifica o layout se for o acesso restrito do montador
...
// FUNÇÃO DE DISPARO DO WHATSAPP (Ajustada para o servidor online)
window.compartilharAgenda = function (nomesEquipe, tipoAgenda) {
  // Usa o link oficial onde a página está rodando
...
async function carregarDashboard() {
  try {
    ...
    if (document.getElementById("cardPendentes")) {
    ...
    if (tabelaHoje) {
      ...
      if (listaHojeFiltrada.length === 0) {
        ...
        const thAcao = nivelAcessoAtual === "montador" ? "" : "<th>Ação</th>";
        const tabelaPai = document
          .querySelector("#tabelaAgendaDia")
          .closest("table");
        if (tabelaPai && tabelaPai.querySelector("thead tr")) {
          tabelaPai.querySelector("thead tr").innerHTML =
    ...
    if (tabelaFutura) {
      ...
      if (listaFuturaFiltrada.length === 0) {
        ...
        const thAcaoFutura =
          nivelAcessoAtual === "montador" ? "" : "<th>Ação</th>";
        const tabelaPaiFutura = document
          .querySelector("#tabelaAgendaFutura")
          .closest("table");
        if (tabelaPaiFutura && tabelaPaiFutura.querySelector("thead tr")) {
          tabelaPaiFutura.querySelector("thead tr").innerHTML =
    ...
    if (document.getElementById("tabelaAgendaDia"))
      document.getElementById("tabelaAgendaDia").innerHTML = erroHtml;
    if (document.getElementById("tabelaAgendaFutura"))
